...
# Initialize host embedding processor
host_embedder = HostEmbedding()
...
async def upload_photos(photos: list[UploadFile] = File(...), ts: str = Form(...), device: str = Form(...)):
    """Handle photo uploads and process through embedding module"""
    ...
    try:
        ...
        # Process photos through host embedding module
        print(f"Processing {len(photos_data)} photos through host embedding module...")
        success = host_embedder.process_photos(photos_data)
        photos = host_embedder.photos_returner(photos_data)
        Facepresence.photos = photos
        
        if not success:
            raise HTTPException(status_code=400, detail="Failed to generate embeddings from photos")
        ...
        print(f"Successfully processed {embedding_count} photos and generated average embedding")
        ...
        return {
            "success": True,
            "message": f"Successfully processed {embedding_count} photos and generated embeddings",
...
async def reset_photos():
    ...
    return {
        "success": True,
        "message": "Photos and embeddings reset successfully",
...
async def get_embedding():
    ...
    if not photos_uploaded or average_embedding is None:
        raise HTTPException(status_code=404, detail="No embedding available. Please upload photos first.")
...
async def get_face_match_info():
    ...
    return {
        ...
        "face_match_info": face_detector.face_matcher.get_match_info()
    }
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Process frame using FacePresence module
